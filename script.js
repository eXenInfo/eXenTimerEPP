document.addEventListener('DOMContentLoaded', () => {
    ...
    if (timerState === 'epp_running_open') {
        ...
        // RESUME: Startet beide Timer wieder, OHNE sie zurückzusetzen
        timerState = 'epp_running_open';

        // Startet den Stations-Timer (Zählt aufwärts) wieder
        stationTimerInterval = setInterval(() => {
            timeLeft++;
            updateTimerDisplay();
        }, 1000);

        // Startet den Gesamt-Timer (Zählt abwärts) wieder
        if (eppGesamtzeit > 0) {
            mainEppTimerInterval = setInterval(() => {
                if (eppGesamtzeit > 0) eppGesamtzeit--;
                if (eppRestzeitAnzeige) eppRestzeitAnzeige.textContent = formatTime(eppGesamtzeit);
    ...
    if (istEppAktiv) {
        ...
        switch (timerState) {
            ...
            case 'epp_running_open':
                ...
                if (stage.pausable) {
                    ...
                    eppPauseBtn.textContent = 'Störung / Pause'; // Text für den laufenden Zustand
                }
                if (timerState === 'epp_running_open') {
                    eppNextStationBtn.classList.remove('hidden');
            ...
            case 'epp_paused':
                ...
                liveTotalProgress.textContent = "Zum Fortsetzen erneut 'Fortsetzen' drücken.";
                 if (stage.pausable) {
                    eppPauseBtn.classList.remove('hidden');
                    eppPauseBtn.textContent = 'Fortsetzen'; // Gleicher Button, neuer Text
                }
                break;
            case 'finished':
        ...
        // Standard-Disziplinen (unverändert)
        switch (timerState) {
            case 'idle':
    ...
    const addStage = () => {
        ...
        if (!duration && duration !== 0) { alert("Bitte geben Sie eine gültige Dauer an."); return; }
        ...
        newStageName.value = ''; newStagePrepTime.value = ''; newStageDuration.value = ''; newStageReps.value = ''; newStagePause.value = '';
        newStageSoundStart.checked = false; newStageSoundEnd.checked = true; newStagePauseAfter.checked = false;
    };
